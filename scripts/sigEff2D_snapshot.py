...
def GetEfficiencies(year,checkPt=False):
    ...
    for snapshot in snapshots:
        ...
        if not TChain.GetEntry(): continue
        if not checkPt:
            # Check efficiency from beginning to end 
            start = TChain.GetLeaf('NPROC').GetValue(0)
            end = TChain.GetLeaf('NMSD').GetValue(0)
        else:
            # Check the effect of the pT cut compared to the previous cut (jetID)
            start = TChain.GetLeaf('NJETS').GetValue(0)
            end = TChain.GetLeaf('NPT').GetValue(0)
        eff = end/start
    ...
    if checkPt:
        plt.savefig('plots/sigEff2D_snapshot_afterPT_{}.pdf'.format(year),dpi=150)
    else:
        plt.savefig('plots/sigEff2D_snapshot_full_{}.pdf'.format(year),dpi=150)

if __name__ == "__main__":
    for year in ['16','16APV','17','18']:
        for checkPT in [True,False]:
            GetEfficiencies(year, checkPt=checkPT)
