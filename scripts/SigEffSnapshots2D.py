...
def GetEfficiencies(year):
    ...
    snapshots = glob.glob('trijet_nano/MX*_{}_snapshot.txt'.format(year))
    # Loop through the snapshots and create a TChain for each
    for snapshot in snapshots:
	print('Opening: {}'.format(snapshot))
	TMass = int(snapshot.split('/')[-1].split('_')[0].split('-')[0].split('MX')[1])
	PhiMass = int(snapshot.split('/')[-1].split('_')[0].split('-')[1].split('MY')[1])
